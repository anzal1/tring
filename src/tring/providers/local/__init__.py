...
These are the providers that make Tring's "local-first" claim real: a complete
...
because :func:`tring.providers.registry._load_builtin` imports it eagerly just
...
**Honest usage.** Every :class:`~tring.providers.base.Usage` here is measured,
...
def _missing(package: str, why: str) -> ImportError:
    ...
    ``tring`` and then pointed an agent spec at ``faster_whisper``.
    """
    return ImportError(
...
class TextInputSTT(STTProvider):
    ...
    the repo. :class:`~tring.runtimes.base.AudioFrame` is just bytes, so a
...
class FasterWhisperSTT(STTProvider):
    ...
    **v0.1 is batch-per-utterance, not streaming.** Frames are buffered until a
    ...
    :class:`~tring.providers.base.STTResult`. No partial hypotheses are emitted.
...
class OllamaLLM(LLMProvider):
    ...
    :class:`~tring.providers.base.LLMChunk` reports ``estimated=False``.
    ...
    def _map_tools(tools: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
        """Adapt Tring tool schemas to Ollama's OpenAI-shaped ``tools`` array.
...
class KokoroTTS(TTSProvider):
    ...
    *buffering policy*. :meth:`synthesize` consumes an async iterator of text
    ...
    def _resample(samples: Any, src_rate: int, dst_rate: int) -> Any:
        ...
        free of a scipy/soxr dependency. A future ``tring[hifi]`` extra should
