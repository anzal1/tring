...
A simple example showing how to run an awaaz agent entirely locally via the
...
Setup:
    1. Install awaaz with local dependencies:
       pip install -e ".[local]"
...
from awaaz.transports.console import ConsoleTransport
...
async def main() -> None:
    ...
    try:
        ...
        print(
            ...
            f"Make sure awaaz is installed with local support. "
            f"({e})",
            file=sys.stderr,
        )
        sys.exit(1)
